...
async function fetchMailchimpStats(accessToken, dc) {
  ...
  const [listsRes, campaignsRes] = await Promise.all([
    ...
    fetch(`${base}/campaigns?count=5&status=sent&fields=campaigns.id,campaigns.settings,campaigns.report_summary,total_items&sort_field=send_time&sort_dir=DESC`, { headers }),
  ]);

  const listsData     = await listsRes.json();
  const campaignsData = await campaignsRes.json();
  const primaryList   = listsData.lists?.[0];
  const lastCampaign  = campaignsData.campaigns?.[0];

  return {
    listName:       primaryList?.name          || '',
    subscribers:    primaryList?.stats?.member_count || 0,
